...
const MainContent = () => {
  ...
  const [moviePopularDatas, setMoviePopularDatas] = useState([])
  ...
  const getMoviePopularData = async ()=>{
    const moviePopularData = await api.get(config.subUrl.movie.popular);

    if (moviePopularData.status === 200) {
      setMoviePopularDatas(moviePopularData.data.results)
    }
  }
