  ...
  return (
    <div className={`lg:hidden  inline-block fixed top-16 w-[80%]  h-full bg-[rgba(3,37,65,.9)] [backdrop-filter:blur(20px)] z-9999 [transition:ease_.3s] p-5 ${toggleBar ? "-left-[80%]":"left-0"}`}>
      <ul className="m-0 p-0 ">
        ...
        <li onClick={()=> setToggleBar(!toggleBar)} className="pb-2.5 "><Link href="/tv" className='text-white font-semibold text-[20.8px] '>TV Shows</Link></li>

        <li onClick={()=> setToggleBar(!toggleBar)} className="pb-2.5 "><Link href="/person" className='text-white font-semibold text-[20.8px] '>People</Link></li>
      </ul>

      <ul className="mt-5 m-0 p-0">
        <li onClick={()=> setToggleBar(!toggleBar)} className="pb-2.5">
      ...
      <ul className="mt-5 m-0 p-0 ">
        <li onClick={()=> setToggleBar(!toggleBar)} className="pb-0 "><Link href="" className='text-[rgba(255,255,255,.6)] text-base font-semibold'>Logout</Link></li>
      </ul>
    </div>
  )
}
